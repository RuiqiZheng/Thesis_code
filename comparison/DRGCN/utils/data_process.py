import csv
from DRGCN.utils import configs as config
import numpy as np
import random
...
def load_data_boundary(dataset, train_ratio, x_flag='feature'):
    ...
    x, adj, adj_norm, label = tuple(objects)
    # label_dense = label.toarray()
    # train_list = []
    #
    # for i in range(0, 663):
    #     temp_index = np.where(label_dense[i] == 1)[0][0]
    #     train_list.append([i, temp_index])
    #
    # np.savetxt(
    #     fname='/RuiqiZheng/undergrad_thesis/undergrad_thesis_code/comparison/DRGCN/dataset/cora/0.3_train_index.txt',
    #     fmt='%i', X=train_list, delimiter=' ')
    train_indexes = []
    label_counts = {}
    balance_num = 0
    with open(os.path.join(dataset, train_ratio + "_train_index.txt"), 'r') as tir:
