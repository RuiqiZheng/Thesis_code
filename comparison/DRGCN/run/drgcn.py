...
x, _, adj_norm, labels, train_indexes, test_indexes, validation_indexes, real_gan_nodes, _, adj_neighbor, all_neighbor_nodes = data_process.load_data_boundary(
    dataset, str(train_ratio),
    x_flag='feature')
# %%
# labels_dense = labels.toarray()
# x_dense = x.toarray()
...
def precision_per_class(preds, labels, mask):
    ...
    for i in val_indexes:
        ...
        predict_list.append(pred_label_i)
        label_list.append(true_label_i)
    accuracy_per_classes = metrics.evaluate(pred_true_labels)
    from sklearn.metrics import classification_report

    # print(pred_true_labels)
    # for _, value in pred_true_labels:
    #     predict_list.append(value[0][0])
    #     label_list.append(value[1][0])

    report = classification_report(label_list, predict_list, output_dict=True)
    ...
    return accuracy_per_classes, roc_auc["micro"], report
...
for epoch in range(epochs):
    ...
    _, train_loss, kl_loss_v, train_acc, train_nn_dl, labeled_samples_run, neighbor_x_val = sess.run(
        (gcn_opt, loss_gcn, kl_loss, accuracy, nn_dl, labeled_samples, neighbor_x), feed_dict=feed_dict_gcn)
    ...
    feed_dict_gan_d.update(({gan_idx: batch_idx, gan_y: y_fb, adj_neighbor_batch: batch_neighbor,
                             real_neighbor_x: neighbor_x_val, real_sample_x: gan_x_val}))
    feed_dict_gan_g.update(({gan_idx: batch_idx, gan_y: y_fb, adj_neighbor_batch: batch_neighbor,
                             real_neighbor_x: neighbor_x_val, real_sample_x: gan_x_val}))
    ...
    if epoch % save_every == 0:
        ...
        train_accuracy_per_classes, _, _ = precision_per_class(train_nn_dl, labels.toarray(), nn_train_mask)
        test_accuracy_per_classes, auc, test_report = precision_per_class(test_nn_dl, labels.toarray(), nn_test_mask)
        ...
        print("test_report_per_classes:{}".format(test_report))
