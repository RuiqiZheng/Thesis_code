...
def load_data( path="dataset/wiki/", dataset="wiki"):
    ...
    majority_test = np.array([x for x in idx_test if labels[x] == 0])
    ...
    return adj, features, labels, idx_train, idx_test, minority, majority, majority_test, minority_test
...
def accuracy_test (output, labels, majority_false):
    preds = output.max(1)[1].type_as(labels)
    labels = labels.cpu()
    preds = preds.cpu()
    preds = preds[np.where(labels == majority_false)[0]]
    labels = labels[np.where(labels == majority_false)[0]]
    correct = preds.eq(labels).double()
    correct = correct.sum()
    return correct / len(labels)
